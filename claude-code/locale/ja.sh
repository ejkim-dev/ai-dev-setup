...
MSG_BREW_DIAGNOSE_SUDO="1. 管理者権限が必要です（最も可能性が高い）"
MSG_BREW_DIAGNOSE_XCODE="2. Xcode Command Line Toolsが未インストール"
MSG_BREW_DIAGNOSE_NETWORK="3. インターネット接続の問題"
MSG_BREW_DIAGNOSE_DISK="4. ディスク容量不足"
MSG_BREW_SOLUTION_SUDO="→ 解決策: sudoで実行: sudo ./setup.sh"
MSG_BREW_SOLUTION_XCODE="→ 解決策: xcode-select --install"
MSG_BREW_SOLUTION_NETWORK="→ 解決策: ネットワーク接続を確認"
MSG_BREW_SOLUTION_DISK="→ 解決策: 最低5GB必要"
MSG_BREW_SKIP_ASK="代替案: Homebrewなしで続行しますか？（手動インストール必要）"
MSG_BREW_SKIP_INFO="後で手動インストール: https://brew.sh"
MSG_BREW_EXIT_RETRY="問題解決後に再実行: ./setup.sh"
MSG_BREW_EXIT_SUDO="または管理者権限で実行: sudo ./setup.sh"
MSG_SUDO_UNAVAILABLE="管理者権限を取得できません"
MSG_SUDO_REQUIRED="Homebrewのインストールには管理者権限が必要な場合があります。"
MSG_SUDO_CONTINUE_ASK="管理者権限なしで続行しますか？（一部機能制限あり）"
MSG_SUDO_EXIT="セットアップをキャンセルします。管理者権限で実行: sudo ./setup.sh"
