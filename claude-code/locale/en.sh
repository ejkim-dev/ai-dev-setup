...
MSG_BREW_DIAGNOSE_SUDO="1. Administrator privileges required (most likely)"
MSG_BREW_DIAGNOSE_XCODE="2. Xcode Command Line Tools not installed"
MSG_BREW_DIAGNOSE_NETWORK="3. Internet connection issue"
MSG_BREW_DIAGNOSE_DISK="4. Insufficient disk space"
MSG_BREW_SOLUTION_SUDO="→ Solution: Run with sudo: sudo ./setup.sh"
MSG_BREW_SOLUTION_XCODE="→ Solution: xcode-select --install"
MSG_BREW_SOLUTION_NETWORK="→ Solution: Check network connection"
MSG_BREW_SOLUTION_DISK="→ Solution: At least 5GB required"
MSG_BREW_SKIP_ASK="Alternative: Continue without Homebrew? (manual installation required)"
MSG_BREW_SKIP_INFO="Manual install later: https://brew.sh"
MSG_BREW_EXIT_RETRY="Re-run after fixing the issue: ./setup.sh"
MSG_BREW_EXIT_SUDO="Or run with administrator privileges: sudo ./setup.sh"
MSG_SUDO_UNAVAILABLE="Cannot obtain administrator privileges"
MSG_SUDO_REQUIRED="Homebrew installation may require administrator privileges."
MSG_SUDO_CONTINUE_ASK="Continue without admin privileges? (some features limited)"
MSG_SUDO_EXIT="Setup canceled. Run with admin privileges: sudo ./setup.sh"
