...
MSG_BREW_DIAGNOSE_SUDO="1. 관리자 권한 필요 (가장 가능성 높음)"
MSG_BREW_DIAGNOSE_XCODE="2. Xcode Command Line Tools 미설치"
MSG_BREW_DIAGNOSE_NETWORK="3. 인터넷 연결 문제"
MSG_BREW_DIAGNOSE_DISK="4. 디스크 공간 부족"
MSG_BREW_SOLUTION_SUDO="→ 해결: sudo로 실행: sudo ./setup.sh"
MSG_BREW_SOLUTION_XCODE="→ 해결: xcode-select --install"
MSG_BREW_SOLUTION_NETWORK="→ 해결: 네트워크 연결 확인"
MSG_BREW_SOLUTION_DISK="→ 해결: 최소 5GB 필요"
MSG_BREW_SKIP_ASK="대안: Homebrew 없이 계속 진행하시겠습니까? (수동 설치 필요)"
MSG_BREW_SKIP_INFO="나중에 수동 설치: https://brew.sh"
MSG_BREW_EXIT_RETRY="문제 해결 후 다시 실행: ./setup.sh"
MSG_BREW_EXIT_SUDO="또는 관리자 권한으로 실행: sudo ./setup.sh"
MSG_SUDO_UNAVAILABLE="관리자 권한을 얻을 수 없습니다"
MSG_SUDO_REQUIRED="Homebrew 설치에 관리자 권한이 필요할 수 있습니다."
MSG_SUDO_CONTINUE_ASK="관리자 권한 없이 계속하시겠습니까? (일부 기능 제한됨)"
MSG_SUDO_EXIT="설정을 중단합니다. 관리자 권한으로 실행: sudo ./setup.sh"
